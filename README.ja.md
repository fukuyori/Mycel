...
- `.mycel/links.json` にファイル同士の関連を保存
...
`--no-mycel` モードでは、`.mycel/order.json`、`.mycel/colors.json`、`.mycel/previews.json`、`.mycel/links.json`、`.mycel/view.json` を読み込まず、作成もしません。このモードでは並び替え、ノードの色変更、ファイル同士の関連付け、表示状態とウィンドウ状態の復元は無効になります。
