...
- プレビュー内テキストの選択とコピー
- 画像や PDF などの簡易プレビュー枠
- 選択した編集可能テキストファイルを直接編集する編集ペイン
- 編集ペインは左、右、下へ配置切り替え可能
- 自動保存、`Ctrl + S` で保存してファイルへフォーカス復帰
...
`--no-mycel` モードでは、これらのファイルを読み込まず、作成もしません。このモードでは並び替え、ノードの色変更、ファイル同士の関連付け、表示状態とウィンドウ状態の復元は無効になります。
...
| 編集ペイン内の Ctrl + S | 保存して編集中ファイルへフォーカスを戻す |
| エディタ内で Ctrl + マウスホイール | 文字サイズを変更 |
