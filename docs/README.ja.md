...
- Phase 3 の一部（入力・ドラッグの一本化）: カードのドラッグ操作の主導権を `NodeItem` の暗黙マウスグラブから `BoardView` へ移し、press / move / release を単一の所有者で扱うように変更。リリース取りこぼしとドラッグ中の再描画クラッシュを解消。

未着手の主な項目: `DropTargetResolver` / `DragController` / `InteractionController` の分離、`NodeItem`・`MainWindow` の軽量化、基本操作の自動テスト（`ctest`）整備。
